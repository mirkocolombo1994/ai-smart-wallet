...
class WalletState extends ChangeNotifier {
    ...
    try {
      ...
      if (goalsString != null && goalsString.isNotEmpty) {
        ...
        _savingsGoals.addAll(
          decoded.map(
            (item) => SavingsGoal.fromJson(item as Map<String, dynamic>),
          ),
        );
      }
  ...
  Future<void> _saveToPrefs() async {
    try {
      ...
      final String serializedGoals = jsonEncode(
        _savingsGoals.map((g) => g.toJson()).toList(),
      );
  ...
  void makeTransactionsRecurring(String description, String recurrence) {
    for (int i = 0; i < _transactions.length; i++) {
      if (!_transactions[i].isProjected &&
          _transactions[i].description.toLowerCase().trim() ==
    ...
    if (index != -1) {
      ...
      if (oldTx.isProjected && oldTx.recurrence != null) {
        ...
        if (oldTx.recurrence == 'weekly') {
          ...
          if (nextDate.month >
              (oldTx.date.month == 12 ? 1 : oldTx.date.month + 1)) {
            nextDate = DateTime(
              oldTx.date.year,
              oldTx.date.month + 2,
              0,
        ...
        final futureTx = Transaction(
          ...
          amount: oldTx
              .amount, // mantiene l'importo fisso originario della previsione
          date: nextDate,
      ...
      _transactions[index] = oldTx.copyWith(
        ...
        clearRecurrence:
            true, // Rimuoviamo la ricorrenza dalla transazione prevista
      );
        ...
        if (tx.isProjected) {
          if (tx.type == TransactionType.income) {
            total += tx.amount;
              ...
              tx.type == TransactionType.expenseCard) {
            total -= tx.amount;
          }
        }
      }
  ...
  double get speseCartaCreditoMesePrecedente {
    ...
    final period = getCreditCardPeriod(DateTime.now());
    final prevPeriod = getCreditCardPeriod(
      period['start']!.subtract(const Duration(days: 1)),
    );
    return getCreditCardExpensesForPeriod(
      prevPeriod['start']!,
      prevPeriod['end']!,
    );
  }
  ...
  List<double> calculateForecastTimeline(int daysHorizon) {
    ...
    for (int day = 1; day <= daysHorizon; day++) {
      ...
      for (var rule in recurringRules) {
        final daysSinceLast = simDate
            .difference(rule['lastDate'] as DateTime)
            .inDays;
      ...
      if (simDate.day == _ccPaymentDay) {
        ...
        bool hasManualCcPayment = _transactions.any(
          (tx) =>
              tx.date.year == simDate.year &&
              tx.date.month == simDate.month &&
              tx.date.day == simDate.day &&
    ...
    for (int day = 1; day <= daysHorizon; day++) {
      ...
      for (var evt in futureEvents) {
        ...
        if (eDate.year == simDate.year &&
            eDate.month == simDate.month &&
            eDate.day == simDate.day) {
